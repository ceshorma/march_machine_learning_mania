    ...
    def build(self, report_date: date | None = None) -> str:
        ...
        if recent:
            for experiment in recent:
                ...
                lines.append(
                    f"- {experiment.get('experiment_id')}: {metric} | "
                    f"improved={experiment.get('improved')} | path={experiment.get('path_key')}"
                )
        else:
        ...
        if pending:
            for hypothesis in sorted(pending, key=lambda item: (-item.priority, item.title.lower())):
                lines.append(
                    f"- [{hypothesis.change_kind}] {hypothesis.title} "
                    f"(priority={hypothesis.priority}, path={hypothesis.path_key})"
                )
        else:
