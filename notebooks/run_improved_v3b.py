"""
March Machine Learning Mania 2026 - Improved v3b
=================================================
Same improvements as v3 but with fillna(0) for stability:
1. BRIER SCORE evaluation (competition metric since 2023)
2. Elo parameter grid search (K, home_adv, carryover)
3. fillna(0) for NaN (stable, no prediction bias)
...
print("=" * 60)
print("MARCH MACHINE LEARNING MANIA 2026 - IMPROVED v3b")
print("=" * 60)
...
def build_team_feats(elo_dict, eff_df, seeds_df, massey_df, conf_df, reg_df,
    ...
    for season, tid in teams_by_season:
        ...
        row["SeedNum"] = sd["SeedNum"].values[0] if len(sd) > 0 else 8.5  # Default for non-tournament teams
...
train = train.fillna(0)  # v3b: fillna(0) for stability (avoids prediction bias)
...
s1_path = SUB_DIR / "submission_stage1_v3b.csv"
...
s2_path = SUB_DIR / "submission_stage2_v3b.csv"
...
results_path = RES_DIR / "results_v3b.json"
